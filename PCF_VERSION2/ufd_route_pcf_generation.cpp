...
Description - Isogen Compliant PCF Generation functions
    This file contains the functions necessary to generate a Piping
    Component File that is compliant with Alias Isogen Isometric drawing 
    package.  The functions implemented here are written with the UG/Open 
    toolkit supplied with Unigraphics.
    For details on the format of a PCF, please consult the Isogen
    documentation.
...
**==========================================================================*/
static int  validate_pcf_file_name( char pcf_file_name[] );
static void handle_status_error( int status );
static int  write_string( char *string, 
                          FILE *file_stream );
static int  close_files( FILE * pcf_stream,
                         FILE * material_stream );
static int  create_component_file( tag_t part_tag,
                                   char *pcf_name);
static int  build_header( tag_t part_tag,
                          FILE * file_stream );
static int build_pipeline_ref( tag_t part_tag,
                               FILE * pcf_stream );
static int build_components( tag_t part_tag,
                             FILE * pcf_stream,
                             FILE * material_stream );
static int get_stock_in_part( tag_t part, 
                              int * num_stocks,
                              tag_t ** stocks );
static int write_component_id( int charx_count,
                               UF_ROUTE_charx_p_t charx_list,
                               FILE * pcf_stream );
static int write_end_points( double end_pt1[3],
                             double end_pt2[3],
                             double inDiameter,
                             double outDiameter,
                             FILE * pcf_stream );
static int write_item_code( int charx_count,
                            UF_ROUTE_charx_p_t charx_list,
                            tag_t comp_tag,
                            FILE * pcf_stream,
                            FILE * material_stream,
                            char ***item_code_char,
                            int *n_items);
static int build_materials( int charx_count,
                            UF_ROUTE_charx_p_t charx_list,
                            tag_t comp_tag,
                            FILE * material_stream );
static int write_weight( int charx_count,
                         UF_ROUTE_charx_p_t charx_list,
                         FILE * pcf_stream );
static int write_skey( int charx_count,
                       UF_ROUTE_charx_p_t charx_list,
                       FILE * pcf_stream );
static int ask_comp_anchors(tag_t part_tag, tag_t comp_tag, 
                            int *num_anchors, tag_t **anchor_tags);
static int write_center_point( tag_t part_tag,
                               tag_t comp_tag,
                               int charx_count,
                               UF_ROUTE_charx_p_t charx_list,
                               FILE * pcf_stream );
static int write_angle( int charx_count,
                        UF_ROUTE_charx_p_t charx_list,
                        FILE * pcf_stream );
static void find_start_and_end_port ( tag_t* port_tags, 
                                      tag_t* start_port,  
                                      tag_t* end_port );
static double find_nps_branch_value ( tag_t* port_tags, 
                                      double inDiameter );
static int find_nps_branch_index ( int charx_count, 
                                   UF_ROUTE_charx_p_t charx_list, 
                                   int* index );
static double determine_in_diameter  ( tag_t comp_tag );
static double determine_out_diameter ( tag_t comp_tag, double inDiameter );
static int write_end_and_branch_points(tag_t part_tag,
                                       tag_t comp_tag,
                                       FILE * pcf_stream );
static void copy_position(double input_pos[3],
                          double output_pos[3]);

static void write_olet_center_and_branch_points ( tag_t comp_tag, 
                                                  FILE * pcf_stream );

static void write_stub_end_point(tag_t comp_tag, FILE * pcf_stream );
static void write_flange_blind_point(tag_t part_tag, tag_t comp_tag, FILE * pcf_stream );

static void write_support_coords ( tag_t part_tag,
                                   tag_t comp_tag,
                                   FILE * pcf_stream );
static void get_lateral_butt_weld_center_point ( double pnt1[3], 
                                                 double pnt2[3], 
                                                 double vec1[3], 
                                                 double vec2[3], 
                                                 double butt_center[3] );

static void get_comp_attr_titles (void);
static void get_item_attr_titles (void);

static bool check_same_point (double point_1[3], double point_2[3]);
static double line_length (double point_1[3], double point_2[3]);
static void coordinate_transform (double input[3], double output[3]);
static void point_direction (double start_point[3], double end_point[3], double direction[3]);
static void getComponents(std::vector<Assemblies::Component*>& components);
static void Splite(string input_str, char* symbol, vector<string>& output_str);
...
static char pcf_comp_attrs[10][32]= {
...
static int n_pcf_comp_attrs = 10;

static char pcf_item_attrs[10][32]= {
    "ITEM-ATTRIBUTE1",
    "ITEM-ATTRIBUTE2",
    "ITEM-ATTRIBUTE3",
    "ITEM-ATTRIBUTE4",
...
static int n_pcf_item_attrs = 10;
...
*
* Input: 
*     none - The PCF is generated based on the current work part when
...
*
* Description: This static function validates a passed part file name.
...
*
* Description: This static function is used by the CHECK_STATUS macro
...
*********************************************************************/
static int get_part_number_charx
( 
    int charx_count,
    UF_ROUTE_charx_p_t charx_list,
    tag_t comp_tag,
 ...
 * and stores the string_values in the static array pcf_comp_attrs
 *
 * Input:
...
* Returns: ERROR_OK or UF_err_operation_aborted
***********************************************************************/
static int add_pipeline_attributes( tag_t part_tag,
                               FILE * pcf_stream )
{
    int i = 0;
    int status = 0;
 ...
 */
static logical item_exists_in_array
(
    tag_t item,
    tag_t *array,
    int num_items
...
/*==========================================================================
  ...
  Return: ERROR_OK or error_code                                         
**==========================================================================*/
static int get_tee_set_on_components( tag_t part, 
                              int * num_rcps,
                              tag_t ** rcps )
{
   int   error_code = 0;
...
* Returns: ERROR_OK or UF_err_operation_aborted
***********************************************************************/
static int get_center_and_branch_points_for_tee_set_on ( tag_t rcp,
                                         int charx_count,
                                         UF_ROUTE_charx_p_t charx_list, 
                                         double center_pt[3],
                                         double branch_pt[3])
...
*/
static void adjust_end_pts_for_tee_set_on 
( 
    tag_t  end_rcps[2], 
    double end_pt1[3], 
    double end_pt2[3] )
...
*/
static int has_part_no_and_material
(
    tag_t comp_tag,
    int charx_count,
    UF_ROUTE_charx_p_t charx_list
 ...
 *  0  -  If invalid 
 */
static int ensure_bolt_udo ( tag_t part_tag, tag_t udo_tag, tag_t gasket )
{
    int is_valid = FALSE;
 ...
 */
 static void get_bolt_length ( int charx_count,
                               UF_ROUTE_charx_p_t charx_list,
 ...
 * Returns
 */
static void apply_component_attribute ( int counter, tag_t obj_tag, char* title, FILE * pcf_stream )
{
      int found = 0;
      char attr_value[ MAX_LINE_BUFSIZE ] ="";
      char pcf_string[ MAX_LINE_BUFSIZE ] ="";
...
*     error code (ERROR_raise) if error 
*************************************************************************/ 
static void find_start_and_end_port(tag_t* port_tags, tag_t* start_port, tag_t* end_port )
{
	UF_ATTR_info_t info;
	logical has_attr = false;
...
*  
*************************************************************************/ 
static double find_nps_branch_value( tag_t* port_tags, double inDiameter )
{
...
* 
*************************************************************************/ 
static void write_flat_direction ( tag_t start_port, tag_t end_port, FILE * pcf_stream )
{
    int status = ERROR_OK;
    double end_pt1[3] = {0};
    double end_pt2[3] = {0};
